...
pub const TimeDeltaError =  error  {
    OutOfRangeError
    // #[allow(deprecated)]
...
pub const  TimeDelta = struct {
    ...
    pub fn new(secs: i64, nanos: u32)  ?TimeDelta {
        if (secs < MIN.secs
            ...
            or (secs == MAX.secs and nanos > MAX.nanos)
            or (secs == MIN.secs and nanos < MIN.nanos)
        )
        {
            return null;
        }
    ...
    pub fn as_seconds_f64(self: Self) f64 {
        return self.secs + self.nanos / NANOS_PER_SEC;
    }

    /// Returns the fractional number of seconds in the `TimeDelta`.
    pub fn as_seconds_f32(self: Self) f32 {
        return self.secs + self.nanos / NANOS_PER_SEC;
    }
