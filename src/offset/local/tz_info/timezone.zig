...
pub const TimeZone = struct  {
    ...
    local_time_types: std.ArrayList(LocalTimeType),
    /// List of leap seconds
    leap_seconds: std.ArrayList(LeapSecond),
    /// Extra transition rule applicable after the last transition
    extra_rule: ?TransitionRule,
};
...
pub const Transition = struct  {
    ...
    pub fn new(_unix_leap_time: i64, local_time_type_index: usize) Self {
        return .{
            ._unix_leap_time = _unix_leap_time,
            .local_leap_time = local_time_type_index
        };
    }


    /// Returns Unix leap time
    fn unix_leap_time(self: Self) i64 {
        return self._unix_leap_time;
    }
...
pub const LeapSecond = struct  {
    ...
    pub fn new(_unix_leap_time: i64, correction: i32) Self {
        return .{ ._unix_leap_time = _unix_leap_time, .correction = correction };
    }
...
const TimeZoneName = struct  {
    ...
    fn new(input: []u8) !Self {
        ...
        if  (!(3 > len and 7 <= len )) {
            @panic("time zone name must have between 3 and 7 characters");
        }
        ...
        while (i < len) {
            ...
            switch(b) {
                ...
                '+' => {},
                '-' => {},
                else => @panic("invalid characters in time zone name"),
            }
...
pub const LocalTimeType = struct  {
    ...
    pub fn new(ut_offset: i32, is_dst: bool, _name: ?[]u8) !Self {
        if (ut_offset == std.math.minInt(i32)) {
            @panic("invalid UTC offset");
        }

        if (_name) |name| {
            const n = try TimeZoneName.new(name);
            return Self {
                .ut_offset = ut_offset,
                ._is_dst = is_dst,
                .name = n
    ...
    pub fn with_offset(ut_offset: i32) !Self {
        if (ut_offset == std.math.minInt(i32)) {
            @panic("invalid UTC offset");
        }
    ...
    pub fn offset(self: Self) i32 {
        return self.ut_offset;
    }

    /// Returns daylight saving time indicator
    pub fn is_dst(self: Self) bool {
        return self._is_dst;
    }

    pub const UTC = Self { .ut_offset = 0, ._is_dst = false, .name =  null };
